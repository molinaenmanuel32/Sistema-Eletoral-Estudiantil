...
public class VotacionRepository
{
    ...
    {
        using var con = DbConnection.GetConnection();
        return con.QueryFirstOrDefault<Votacion>("SELECT * FROM Votaciones WHERE VotacionId = @Id", new { Id = id });
    }
...
public class VotoRepository
{
    ...
    public bool UsuarioYaVoto(int votacionId, int padronId)
    {
        using var con = DbConnection.GetConnection();
        var voto = GetVotoByPadron(votacionId, padronId);
        // Puede votar si no ha votado, o si su voto es nulo (por tiempo)
        if (voto is null) return false;
        return !voto.EsNulo;
    }
    ...
    public EstadisticasVotacion GetEstadisticas(int votacionId)
    {
        ...
        var result = con.QueryMultiple("sp_EstadisticasVotacion", new { VotacionId = votacionId },
                                       commandType: System.Data.CommandType.StoredProcedure);
    ...
    {
        using var con = DbConnection.GetConnection();
        con.Execute("sp_MarcarVotosNulos", new { VotacionId = votacionId },
                    commandType: System.Data.CommandType.StoredProcedure);
        return true;
    }

    // Reporte general de votos (anónimo – solo indica si votó y si fue nulo)
    public IEnumerable<dynamic> GetReporteGeneral(int votacionId)
    {
        using var con = DbConnection.GetConnection();
        const string sql = """
...
public class AuditoriaRepository
{
    ...
    public IEnumerable<LogAuditoria> GetRecientes(int top = 100)
    {
        using var con = DbConnection.GetConnection();
        return con.Query<LogAuditoria>(
            $"SELECT TOP {top} * FROM LogAuditoria ORDER BY Fecha DESC");
